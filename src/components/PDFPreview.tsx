...
export function PDFPreview({ vertebraeData, carePhases, practiceName, clientName = "", clientEmail = "", providerName = "", reportDate, open, onOpenChange }: PDFPreviewProps) {
  ...
  const generatePreview = async () => {
    ...
    const drawSpineDiagram = (selectedIds: string[], centerX: number, startY: number) => {
      const allVertebrae = [
    ...
    const spineEndY = drawSpineDiagram(vertebraeData.map(v => v.id), pageWidth / 2, spineStartY)
    yPos = spineEndY + 3
